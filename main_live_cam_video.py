...
class ExpressionApp(QWidget):
    def __init__(self):
        ...
        self.original_label.setMinimumWidth(500)
        ...
        top_buttons.addWidget(self.btn_camera)
        ...
        main_hbox.addWidget(self.original_label, 2)
        main_hbox.addLayout(right_vbox, 3)
    ...
    def add_progress_bar(self, emotion, color=None):
        ...
        percent = QLabel("0.0000%")
        percent.setFixedWidth(80)
    ...
    def process_frame(self, image):
        ...
        self.small_orig_label.setPixmap(pixmap.scaled(self.small_orig_label.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        if not check:
            ...
            if self.missing_face_counter >= 10:
                ...
                if self.camera_mode:
                    ...
                    QMessageBox.information(self, "Camera Stopped", "Face not detected for 10 frames. Camera stopped.")
            return
    ...
    def update_results(self, prediction):
        if prediction is None:
            for emo in self.exp_dict.values():
                ...
                self.percentage_labels[emo].setText("0.0000%")
            self.progress_bars['No Face'].setValue(100)
            self.percentage_labels['No Face'].setText("100.0000%")
        ...
        for emo_label, value in zip(self.exp_dict.values(), prob):
            ...
            self.percentage_labels[emo_label].setText(f"{percent:.4f}%")
        self.progress_bars['No Face'].setValue(0)
        self.percentage_labels['No Face'].setText("0.0000%")

        pred_idx = list(self.exp_dict.values()).index(label)
        self.result_label.setText(
            f"<b>Prediction: <span style='color:#ff9500'>{label}</span> ({prob[pred_idx]*100:.4f}%)</b>"
        )
        result_json = {
