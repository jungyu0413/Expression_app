...
class ExpressionApp(QWidget):
    def __init__(self):
        ...
        self.scroll_area.setStyleSheet("background-color: #ffffff; border: 1px solid #ccc;")

        thumb_container = QVBoxLayout()
        thumb_container.addWidget(self.small_orig_label)
        thumb_container.addWidget(self.scroll_area)
    ...
    def on_face_selected(self, idx):
        ...
        self.result_label.setText(
            f"<b>Prediction: <span style='color:#ff9500'>{label}</span></b><br>Confidence: {prob[pred_cls]*100:.4f}%")
        ...
        for i, emotion in self.exp_dict.items():
            value = prob[i] * 100
            self.progress_bars[emotion].setValue(int(value))
            self.percentage_labels[emotion].setText(f"{value:.4f}%")
